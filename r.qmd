...
```{r}
#| include: false
# Figures for a page in light or dark (2026-08-29): no device background; text, axes, and points in a grey that reads on either ground.
knitr::opts_chunk$set(dev.args = list(bg = "transparent"))
setHook("before.plot.new", function() par(fg = "#7b8698", col = "#7b8698", col.axis = "#7b8698", col.lab = "#7b8698", col.main = "#7b8698", col.sub = "#7b8698"))
setHook(packageEvent("ggplot2", "attach"), function(...) ggplot2::theme_update(
  plot.background = ggplot2::element_blank(), legend.background = ggplot2::element_blank(),
  text = ggplot2::element_text(colour = "#7b8698"), axis.text = ggplot2::element_text(colour = "#7b8698")))
```
...
We can use the `broom.mixed::tidy()` function to get a tidy table of the results. This will prove to be super useful later when we need to combine the output from hundreds of simulations to calculate power. We will add columns for `parameter` and `value`, so we can compare the estimate from the model to the parameters we used to simulate the data. 
...
```{r}
sims <- future_map_dfr(1:reps, ~ single_run(), .options = furrr_options(seed = TRUE))
```

We can finally calculate power for our parameter of interest `beta_1`(denoted in the tidy model output table as the term `genre_i`) by filtering to keep only that term and then calculating the proportion of times the $p$-value is below the `alpha` (`r alpha`) threshold.
...
sims_fp <- future_map_dfr(1:reps, ~ single_run(beta_1 = 0), .options = furrr_options(seed = TRUE))
