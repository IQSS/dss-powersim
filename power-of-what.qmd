...
```{r}
#| include: false
pacman::p_load(tidyverse, papaja, knitr)  # the two parameter tables below (apa_table) and the pipe
```
...
The study design we will use as an example throughout this tutorial comes from Julian Quandt's blogpost (<https://julianquandt.com/posts/2020-08-05-power-analysis-by-data-simulation-in-r-part-iv.html>). He describes this as:
...
where the subscripts $i$ and $j$ denote individual songs and participants, respectively, `liking` is an integer-based rating of a given song on the interval [0, 100], `genre` is a dummy coded binary variable indicating whether the song is classified as "rock" or "pop", and we assume the two by-subject effects are drawn together from a bivariate normal, $\begin{pmatrix} T_{0j} \\ T_{1j} \end{pmatrix} \sim \mathcal{N}\left( \begin{pmatrix} 0 \\ 0 \end{pmatrix}, \begin{pmatrix} \tau_0^2 & \rho\tau_0\tau_1 \\ \rho\tau_0\tau_1 & \tau_1^2 \end{pmatrix} \right)$, with $O_{0i} \sim \mathcal{N}(0, \omega_0)$ and $\epsilon_{ij} \sim \mathcal{N}(0, \sigma)$ (the second argument in each case is a standard deviation, not a variance). The parameter of interest is $\beta_1$ - the average (within-subject) difference in the rating of songs between the two genres. Table 3.1 lists all of the variables and parameters in the model. 
...
tibble::tribble(~`model`, ~`code`, ~`description`,
        "$\\textrm{liking}_{ij}$", "`liking_ij`", "rating of song $i$ for participant $j$ on the interval [0, 100]",
        "$\\textrm{genre}_i$", "`genre_i`", "genre of song $i$ (0='pop', 1='rock')",
        "$\\beta_0$", "`beta_0`", "intercept; mean of liking rating for 'pop' genre",
        "$\\beta_1$", "`beta_1`", "slope; mean difference btw 'pop' and 'rock' song ratings",
        "$\\tau_0$", "`tau_0`", "standard deviation of by-subject random intercepts",
        "$\\tau_1$", "`tau_1`", "standard deviation of by-subject random slopes",        
        "$\\rho$", "`rho`", "correlation between by-subject random intercepts and slopes",
        "$\\omega_0$", "`omega_0`", "standard deviation of by-song random intercepts",
        "$\\sigma$", "`sigma`", "standard deviation of residuals",
        "$T_{0j}$", "`T_0j`", "random intercept for subject $j$",
        "$T_{1j}$", "`T_1j`", "random slope for subject $j$",
        "$O_{0i}$", "`O_0i`", "random intercept for song $i$",
        "$e_{ij}$", "`e_ij`", "residual of song $i$ for participant $j$") %>%
...
tibble::tribble(~`code`, ~`value`, ~`description`,
        "`beta_0`", "60", "intercept; i.e., mean of liking rating for 'pop' genre",
        "`beta_1`", "5", "slope; i.e, mean difference btw 'pop' and 'rock' song ratings",
