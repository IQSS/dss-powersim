...
```{python}
#| include: false
# Figures for a page in light or dark (2026-08-29): no figure background; text, axes, and ticks in a grey that reads on either ground.
import matplotlib as mpl
mpl.rcParams.update({"figure.facecolor": "none", "savefig.facecolor": "none", "legend.facecolor": "none",
                     "text.color": "#7b8698", "axes.labelcolor": "#7b8698", "axes.edgecolor": "#7b8698",
                     "xtick.color": "#7b8698", "ytick.color": "#7b8698"})
```
...
::: {.callout-note title="Two differences from the R chapter"}

1. R uses `p_load()` to install any missing libraries and load them in one step. In Python the environment is configured separately --- here by `uv sync` from the materials folder --- so the imports below assume the packages are already installed.
2. The R chapter sets up parallelism once in its setup section; this chapter uses `dask` at the point where the simulations are run.
...
1. The mixed model is fitted with `statsmodels`, which does not report the correlation between a subject's random intercept and random slope, so `rho` cannot be recovered from the fit as it can in R.
2. There is no equivalent of `broom.mixed::tidy()`, so the results are pulled out of the summary table by hand rather than arriving as a tidy data frame.
...
1. R loops with `future_map_dfr()`; here a plain `for` loop builds a list of delayed calls.
2. R configures parallelism once at the top; here `dask` does it at the point of use.
...
```
We can finally calculate power for our parameter of interest $\beta_1$ denoted in the model output table as the term $genre_{i}$ by filtering to keep only that term and then calculating the proportion of times the $p$-value is below the alpha (0.05) threshold.
